...
module.exports = async (client, interaction) => {
  ...
  if(interaction.customId != "ticketclose") return;
  ...
  schema.findOne({ guildID: guild.id, ticketID: ticketid}, async(err, data) => {
      ...
      if(data) {  
         const row = new MessageActionRow()
              .addComponents(
                new MessageButton()
                  .setCustomId('ticketcloseyes')
                  .setLabel('Oui')
                  .setStyle('SUCCESS'),
                new MessageButton()
                  .setCustomId('ticketcloseno')
                  .setLabel('Non')
                  .setStyle('DANGER'),
              );
        ...
        interaction.message.channel.send({embeds:[close], components: [row]})
      } else {
