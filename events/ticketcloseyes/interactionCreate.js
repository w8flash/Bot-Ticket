const { Message, MessageEmbed, MessageAttachment, ButtonInteraction } = require("discord.js");
...
module.exports = async (client, interaction) => {
  ...
  if(interaction.customId != "ticketcloseyes") return;
  ...
  schema.findOne({ guildID: guild.id, ticketID: ticketid}, async(err, data) => {
      ...
      if(data) {  
        ...
        let close = new MessageEmbed()
          ...
          .setDescription("Le ticket s'auto détruiras dans 5 secondes...")
        ...
        setTimeout(() => {
          interaction.channel.delete().then(async ch => {
            client.ticketTranscript.findOne({ channel: ch.id }, async(err, data) => {
              if(err) throw err;
              if(data){
                await schema.findOneAndDelete({ id: userID })
                  .catch(err => console.log(err))
                const cha = ch.id;
                fs.writeFileSync(`../Bot-Ticket/tickets/${ch.id}.txt`, data.Content.join("\n\n"));
                //transcriptChannel.send(`Le ticket de ${message.guild.members.cache.get(ch.name).user.username} a été fermé.`);
                if(logsChannel) logsChannel.send({embeds:[deletedEmbed], files: [fs.createReadStream(`../Bot-Ticket/tickets/${ch.id}.txt`)]});
                //await logsChannel.send(new MessageAttachment(fs.createReadStream(`../Bot-Ticket/tickets/${ch.id}.txt`)));
                client.ticketTranscript.findOneAndDelete({ channel: cha});
              }
            })
          })
        }, 5000)
